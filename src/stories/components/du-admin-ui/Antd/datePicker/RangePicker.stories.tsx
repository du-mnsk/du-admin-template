...
// 스토리북에서 컴포넌트 이름을 "Antd.RangePicker"로 표시
if (Antd.RangePicker) {
  Antd.RangePicker.displayName = 'Antd.RangePicker'
}

const meta = {
  title: 'Common/Antd/Antd.RangePicker',
  component: Antd.RangePicker,
  parameters: {
    layout: 'centered',
  },
  tags: ['autodocs'],
  argTypes: {
    ...
    mode: {
      ...
      description: '패널 모드([string, string])',
    },
    nextIcon: {
    ...
    placeholder: {
      control: 'text',
      description: '플레이스홀더 배열(string | [string, string])',
    },
    placement: {
      control: 'select',
    ...
    allowEmpty: {
      control: 'object',
      description: '빈 값 허용 여부 [boolean, boolean]',
    },
    defaultPickerValue: {
      description: '기본 패널 날짜 [Dayjs, Dayjs]',
    },
    defaultValue: {
      description: '기본 선택 날짜 범위 [Dayjs, Dayjs]',
    },
    disabledTime: {
      description: '비활성화할 시간 함수((date: Dayjs, type: "start" | "end") => DisabledTime)',
    },
    format: {
      control: 'text',
      description: '날짜 형식(string | string[])',
    },
    ranges: {
      control: 'object',
      description: '빠른 선택 옵션 객체',
    },
    renderExtraFooter: {
    ...
    value: {
      description: '선택된 날짜 범위 [Dayjs, Dayjs]',
    },
    onCalendarChange: {
      action: 'calendar-changed',
      description: '캘린더 날짜 변경 이벤트 핸들러',
    },
    onChange: {
      action: 'changed',
      description: '날짜 범위 변경 이벤트 핸들러',
    },
  },
} satisfies Meta<typeof Antd.RangePicker>
...
export const WithValue: Story = {
  ...
  render: function WithValueStory(args) {
    const [value, setValue] = useState<[Dayjs | null, Dayjs | null] | null>(
      args.value || [dayjs('2024-01-01'), dayjs('2024-01-31')]
    )
    ...
    return (
      <Antd.RangePicker
        ...
        onChange={(dates, dateStrings) => {
          setValue(dates)
          args.onChange?.(dates, dateStrings)
        }}
      />
    )
  },
...
export const WithDefaultPickerValue: Story = {
  args: {
    defaultPickerValue: [dayjs('2024-01-01'), dayjs('2024-01-31')],
  },
}
...
export const WithRanges: Story = {
  args: {
    ranges: {
      '오늘': [dayjs(), dayjs()],
      '이번 주': [dayjs().startOf('week'), dayjs().endOf('week')],
      '이번 달': [dayjs().startOf('month'), dayjs().endOf('month')],
      '이번 년': [dayjs().startOf('year'), dayjs().endOf('year')],
    },
  },
}
...
export const WithMode: Story = {
  args: {
    ...
    picker: 'year',
    format: 'YYYY',
    defaultValue: [dayjs('2020'), dayjs('2024')],
  },
}
...
export const WithOnCalendarChange: Story = {
  args: {
    defaultValue: [dayjs('2024-01-01'), dayjs('2024-01-31')],
  },
  render: function WithOnCalendarChangeStory(args) {
    const [value, setValue] = useState<[Dayjs | null, Dayjs | null] | null>(
      args.defaultValue || [dayjs('2024-01-01'), dayjs('2024-01-31')]
    )
    ...
    return (
      <Antd.RangePicker
        ...
        onCalendarChange={(dates, dateStrings, info) => {
          args.onCalendarChange?.(dates, dateStrings, info)
        }}
        onChange={(dates, dateStrings) => {
          setValue(dates)
          args.onChange?.(dates, dateStrings)
...
export const MonthRangePicker: Story = {
  args: {
    picker: 'month',
    format: 'YYYY-MM',
    defaultValue: [dayjs('2024-01'), dayjs('2024-12')],
  },
}

// picker=week
export const WeekRangePicker: Story = {
  args: {
    picker: 'week',
    format: 'YYYY-wo',
    defaultValue: [dayjs('2024-01-01'), dayjs('2024-01-07')],
  },
}
