import { useState } from 'react'
import { EyeInvisibleOutlined, EyeTwoTone } from '@ant-design/icons'
import type { Meta, StoryObj } from '@storybook/react-vite'

import { Antd } from '@/shared/components/du-admin-ui/Antd'

// 스토리북에서 컴포넌트 이름을 "Antd.InputPassword"로 표시
if (Antd.InputPassword) {
  Antd.InputPassword.displayName = 'Antd.InputPassword'
}

const meta = {
  title: 'Common/Antd/Antd.InputPassword',
  component: Antd.InputPassword,
  parameters: {
    layout: 'centered',
  },
  tags: ['autodocs'],
  argTypes: {
    addonAfter: {
      description: '입력 후 추가 요소(ReactNode)',
    },
    addonBefore: {
      description: '입력 전 추가 요소(ReactNode)',
    },
    allowClear: {
      control: 'boolean',
      description: '입력 초기화 버튼 표시 여부',
    },
    autoFocus: {
      control: 'boolean',
      description: '자동 포커스 여부',
    },
    bordered: {
      control: 'boolean',
      description: '테두리 표시 여부',
    },
    className: {
      control: 'text',
      description: '클래스명',
    },
    defaultValue: {
      control: 'text',
      description: '기본 값',
    },
    disabled: {
      control: 'boolean',
      description: '입력 비활성화 여부',
    },
    iconRender: {
      description: '비밀번호 표시/숨김 아이콘 커스텀 렌더링 함수((visible: boolean) => ReactNode)',
    },
    id: {
      control: 'text',
      description: '입력 필드 ID',
    },
    maxLength: {
      control: 'number',
      description: '최대 입력 길이',
    },
    prefix: {
      description: '입력 접두사(ReactNode)',
    },
    readOnly: {
      control: 'boolean',
      description: '입력 읽기 전용 여부',
    },
    size: {
      control: 'select',
      options: ['large', 'middle', 'small'],
      description: '입력 크기',
    },
    status: {
      control: 'select',
      options: ['error', 'warning'],
      description: '입력 상태',
    },
    suffix: {
      description: '입력 접미사(ReactNode)',
    },
    value: {
      control: 'text',
      description: '입력 값',
    },
    visibilityToggle: {
      control: 'boolean',
      description: '비밀번호 표시/숨김 기능 활성화 여부',
    },
    placeholder: {
      control: 'text',
      description: '플레이스홀더',
    },
    onPressEnter: {
      action: 'pressed-enter',
      description: 'Enter 키 입력 이벤트 핸들러',
    },
    onChange: {
      action: 'changed',
      description: '입력 변경 이벤트 핸들러',
    },
    onBlur: {
      action: 'blurred',
      description: '포커스 아웃 이벤트 핸들러',
    },
    onFocus: {
      action: 'focused',
      description: '포커스 인 이벤트 핸들러',
    },
    onKeyDown: {
      action: 'key-down',
      description: '키 다운 이벤트 핸들러',
    },
  },
} satisfies Meta<typeof Antd.InputPassword>

export default meta
type Story = StoryObj<typeof meta>

export const Default: Story = {
  args: {},
}

export const WithPlaceholder: Story = {
  args: {
    placeholder: '비밀번호를 입력하세요',
  },
}

export const WithDefaultValue: Story = {
  args: {
    defaultValue: 'password123',
  },
}

export const WithValue: Story = {
  args: {
    value: 'password123',
  },
  render: function WithValueStory(args) {
    const [value, setValue] = useState<string>(args.value as string || '')
    
    return (
      <Antd.InputPassword
        {...args}
        value={value}
        onChange={(e) => {
          setValue(e.target.value)
          args.onChange?.(e)
        }}
      />
    )
  },
}

export const Disabled: Story = {
  args: {
    disabled: true,
    defaultValue: 'password123',
  },
}

export const ReadOnly: Story = {
  args: {
    readOnly: true,
    defaultValue: 'password123',
  },
}

export const WithMaxLength: Story = {
  args: {
    maxLength: 20,
    placeholder: '최대 20자까지 입력 가능',
  },
}

export const WithPrefix: Story = {
  args: {
    prefix: '🔒',
    placeholder: '비밀번호를 입력하세요',
  },
}

export const Large: Story = {
  args: {
    size: 'large',
    placeholder: '큰 크기 비밀번호 입력',
  },
}

export const Small: Story = {
  args: {
    size: 'small',
    placeholder: '작은 크기 비밀번호 입력',
  },
}

export const WithStatusError: Story = {
  args: {
    status: 'error',
    placeholder: '에러 상태 비밀번호 입력',
  },
}

export const WithStatusWarning: Story = {
  args: {
    status: 'warning',
    placeholder: '경고 상태 비밀번호 입력',
  },
}

export const WithoutBordered: Story = {
  args: {
    bordered: false,
    placeholder: '테두리 없는 비밀번호 입력',
  },
}

export const AllowClear: Story = {
  args: {
    allowClear: true,
    defaultValue: 'password123',
  },
}

export const WithoutVisibilityToggle: Story = {
  args: {
    visibilityToggle: false,
    placeholder: '비밀번호 표시/숨김 기능 없음',
  },
}

export const WithCustomIcon: Story = {
  args: {
    iconRender: (visible) => (visible ? <EyeTwoTone /> : <EyeInvisibleOutlined />),
    placeholder: '커스텀 아이콘 사용',
  },
}

export const AutoFocus: Story = {
  args: {
    autoFocus: true,
    placeholder: '자동 포커스',
  },
}

export const WithOnPressEnter: Story = {
  args: {
    placeholder: 'Enter 키를 눌러보세요',
    onPressEnter: (e) => {
      alert('Enter 키가 눌렸습니다!')
    },
  },
}

export const WithAddonBefore: Story = {
  args: {
    addonBefore: 'Password:',
    placeholder: '비밀번호를 입력하세요',
  },
}

export const WithAddonAfter: Story = {
  args: {
    addonAfter: '🔐',
    placeholder: '비밀번호를 입력하세요',
  },
}

export const WithAddonBeforeAndAfter: Story = {
  args: {
    addonBefore: 'Password:',
    addonAfter: '🔐',
    placeholder: '비밀번호를 입력하세요',
  },
}
